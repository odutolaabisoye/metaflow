  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-white/40">{{ rangeLabel }} · scored daily by ROAS, CTR, margin &amp; inventory</p>
      </div>
      <div class="flex items-center gap-2.5">
        ...
        <button class="flex items-center gap-2 rounded-xl bg-white text-ink-950 px-4 py-2.5 text-sm font-semibold hover:bg-white/90 transition-all">
    ...
    <div class="glass rounded-2xl p-4 flex flex-wrap items-center gap-3">
      ...
      <div class="relative flex-1 min-w-[200px] max-w-xs">
        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/30 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        ...
        <input
          ...
          class="w-full rounded-xl border border-white/10 bg-white/[0.06] pl-9 pr-4 py-2.5 text-sm text-white placeholder:text-white/25 outline-none focus:border-glow-500/40 focus:ring-2 focus:ring-glow-500/15 transition-all"
        />
      </div>
      ...
      <div class="flex items-center gap-2">
        <span class="text-xs text-white/30">Sort by</span>
        <select
          v-model="sortBy"
        ...
        <button
          ...
          class="flex items-center justify-center w-8 h-8 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-white hover:bg-white/10 transition-all"
        >
    ...
    <div class="glass rounded-2xl overflow-hidden">
      ...
      <div v-else-if="!products.length" class="p-16 text-center">
        ...
        <p class="mt-1 text-xs text-white/40">Try adjusting your search or clearing the filters</p>
        <button @click="filter = 'all'; search = ''" class="mt-4 text-xs text-glow-500 hover:text-glow-400 transition-colors">
          Clear filters →
        </button>
      </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-white/10">
              <th class="text-left text-xs font-medium text-white/35 px-5 py-3.5 whitespace-nowrap">Product</th>
              <th class="text-left px-4 py-3.5 whitespace-nowrap">
                <button class="flex items-center gap-1.5 text-xs font-medium text-white/35 hover:text-white/60 transition-colors" @click="setSort('score')">
                  Score
                  <span v-if="sortBy === 'score'" class="text-glow-500 text-[10px]">{{ sortIndicator('score') }}</span>
                </button>
              </th>
              <th class="text-left px-4 py-3.5 whitespace-nowrap">
                <button class="flex items-center gap-1.5 text-xs font-medium text-white/35 hover:text-white/60 transition-colors" @click="setSort('roas')">
                  ROAS
                  <span v-if="sortBy === 'roas'" class="text-glow-500 text-[10px]">{{ sortIndicator('roas') }}</span>
                </button>
              </th>
              <th class="text-left px-4 py-3.5 whitespace-nowrap">
                <button class="flex items-center gap-1.5 text-xs font-medium text-white/35 hover:text-white/60 transition-colors" @click="setSort('ctr')">
                  CTR
                  <span v-if="sortBy === 'ctr'" class="text-glow-500 text-[10px]">{{ sortIndicator('ctr') }}</span>
                </button>
              </th>
              <th class="text-left px-4 py-3.5 whitespace-nowrap">
                <button class="flex items-center gap-1.5 text-xs font-medium text-white/35 hover:text-white/60 transition-colors" @click="setSort('margin')">
                  Margin
                  <span v-if="sortBy === 'margin'" class="text-glow-500 text-[10px]">{{ sortIndicator('margin') }}</span>
                </button>
              </th>
              <th class="text-left px-4 py-3.5 whitespace-nowrap">
                <button class="flex items-center gap-1.5 text-xs font-medium text-white/35 hover:text-white/60 transition-colors" @click="setSort('revenue')">
                  Revenue
                  <span v-if="sortBy === 'revenue'" class="text-glow-500 text-[10px]">{{ sortIndicator('revenue') }}</span>
                </button>
              </th>
              <th class="text-left px-4 py-3.5 whitespace-nowrap text-xs font-medium text-white/35">Status</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-white/[0.05]">
            ...
            >
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="min-w-0">
                    ...
                    <div class="flex items-center gap-2 mt-0.5">
                      <span class="text-xs text-white/35 font-mono">{{ item.sku }}</span>
                      <a
                        :href="item.productUrl"
                        target="_blank"
                        rel="noopener"
                        class="text-xs text-glow-500/70 hover:text-glow-500 transition-colors opacity-0 group-hover:opacity-100"
                      >↗</a>
                    </div>
                  </div>
                </div>
              ...
              <!-- CTR -->
              <td class="px-4 py-4 text-sm text-white/70">{{ item.ctr }}%</td>

              <!-- Margin -->
              <td class="px-4 py-4 text-sm text-white/70">{{ item.margin }}%</td>
      ...
      <div v-if="!pending && products.length > 0" class="flex items-center justify-between px-5 py-4 border-t border-white/10">
        <div class="flex items-center gap-2">
          <span class="text-xs text-white/35">Rows</span>
          <select
        ...
        <div class="flex items-center gap-3">
          <span class="text-xs text-white/35">{{ products.length }} of {{ total }} products</span>
          <div class="flex items-center gap-1.5">
            <button
              :disabled="pageStack.length === 0"
              @click="prevPage"
              class="flex items-center justify-center w-8 h-8 rounded-lg border border-white/10 bg-white/5 text-white/50 hover:text-white hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
            >
